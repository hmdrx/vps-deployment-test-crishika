...
export const getAllQuestion = catchAsync(
  async (req: Request, res: Response, _next: NextFunction) => {
    ...
    if (req.query.amount) {
      allQuestion = await Question.find().limit(+req.query.amount);
    } else {
      console.log('im working');
      allQuestion = await Question.find();
    }
...
export const deleteQuestion = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const questionId = (await Question.findById(req.params.id)) as IQuestion;
    if (!questionId) next(new AppError('no question found to delete', 404));

    await Question.findOneAndDelete(questionId._id);
    res.status(204).json({
      status: 'success',
      message: 'deleted',
    });
